import { FiMessageSquare, FiHeart, FiShare2 } from "react-icons/fi"; //libreria de iconos
...
export const PostCard = ({ post }: PostCardProps) => {
  ...
  const nicknameValido = post.userNickname || "Anónimo"; // por si el backend no manda el usernickname o viene null

  // Avatars de colores para usuarios, se genera un color de fondo aleatorio basado en la suma de los caracteres del nickname
  const bgColors = ["bg-blue-500", "bg-purple-500", "bg-pink-500", "bg-green-500", "bg-yellow-500"];
  const charCodeSum = nicknameValido.split("").reduce((acc, char) => acc + char.charCodeAt(0), 0); // esto para que le de un color unico a cada usuario usnado codigo ASCII
    ...
    <article className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700/50 rounded-xl p-5 mb-4 shadow-sm hover:shadow-md dark:hover:bg-gray-800/80 transition-all duration-200 cursor-pointer">
      <div className="flex gap-3.5">
        ...
        <div className={`w-12 h-12 rounded-full ${avatarBg} text-white flex items-center justify-center font-bold text-base shadow-inner shrink-0`}>
          {/* Tomamos las primeras dos letras del nickname */}
        ...
        <div className="flex-1">
          ...
          {/* Acciones del Post hay que ver si las dejamos !!!!!!!!!!!! */}
